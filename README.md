...
| `SANDBOX_API_IDLE_STOP_AFTER` | *(unset / off)* | After this duration since `last_active_at`, the API calls runner **StopSandbox** (container kept). Go [`time.ParseDuration`](https://pkg.go.dev/time#ParseDuration) (e.g. `30m`, `2h`). `0` means the same as unset (idle stop disabled). |
| `SANDBOX_API_IDLE_DELETE_AFTER` | *(unset / off)* | After this duration since `last_active_at`, proxied routes and `GET /sandboxes/{id}` return **404** (same as missing sandbox) until the sweeper removes the runner container and DB row after the safety buffer. `0` means the same as unset (idle delete disabled). |
