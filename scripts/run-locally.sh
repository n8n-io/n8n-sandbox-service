...
if [[ "${SANDBOX_COMPOSE_TLS:-1}" != "0" ]]; then
	...
	if [[ ! -f .tls/grpc-server.crt ]] || [[ ! -f .tls/control-grpc-server.crt ]]; then
		echo "Expected .tls/grpc-server.crt and .tls/control-grpc-server.crt after bootstrap. Install openssl or set SANDBOX_COMPOSE_TLS=0 for plaintext gRPC."
		exit 1
	fi
