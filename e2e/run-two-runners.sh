...
docker run -d \
	...
	-e "SANDBOX_RUNNER_HTTP_BASE_URL=https://${RUNNER1_CONTROL_ALIAS}:8080" \
...
docker run -d \
	...
	-e "SANDBOX_RUNNER_REGISTRATION_GRPC_KEY_FILE=/grpc-tls/grpc-client.key" \
	-e "SANDBOX_RUNNER_REGISTRATION_GRPC_SERVER_NAME=$API_TLS_DNS" \
	-e "SANDBOX_RUNNER_HTTP_BASE_URL=https://${RUNNER2_CONTROL_ALIAS}:8080" \
	-e "SANDBOX_RUNNER_CONTROL_GRPC_LISTEN_ADDR=:9091" \
...
wait_runner() {
	...
	for i in $(seq 1 60); do
		if docker exec "$name" wget -q -O - --no-check-certificate https://localhost:8080/readyz >/dev/null 2>&1; then
			echo "$name is ready."
			return 0
		fi
		if [[ "$i" -eq 60 ]]; then
