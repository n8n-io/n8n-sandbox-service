...
- **[sysbox-runc](https://github.com/nestybox/sysbox)** — a container runtime that enables secure Docker-in-Docker without `--privileged`. Required on Linux for production-grade isolation. See the [Linux quickstart](quickstart-linux.md) for installation instructions.
  ...
  - **Not supported on immutable-rootfs distributions** (Talos, Bottlerocket, Flatcar, Fedora CoreOS). Sysbox must write to the node filesystem and the containerd configuration. On Kubernetes, the Helm chart offers `runner.isolation: privileged` (weaker isolation) for those clusters. See the [Kubernetes quickstart](quickstart-k8s.md#immutable-rootfs-distributions-privileged-isolation).
