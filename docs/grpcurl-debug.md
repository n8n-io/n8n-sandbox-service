...
```bash
openssl s_client -connect localhost:9090 -servername n8n-sandbox-service-api-local -brief </dev/null
```
...
```bash
grpcurl -import-path "$PROTO_IMPORT" -proto "$PROTO_FILE" list n8n-sandbox-service-runner-dind-local-1:9091
```
